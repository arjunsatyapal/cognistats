...
public class ChoiceReactionTimeStatistics {
	...
	public void updateStatistics(ChoiceReactionTimeTrialResult trialResult) {
		...
		if (trialResult.clickedTooEarly) {
			...
			if (trialResult.userAnswer == trialResult.correctAnswer) {
				...
				sumCorrectReactionTimesSquared += (trialResult.userReactionTime) * (trialResult.userReactionTime);
			}
			else {
				++wrongTrials;
			}
	...
	public double getStdDevReactionTime() {
		...
		return Math.sqrt(((double)(sumCorrectReactionTimesSquared - sumCorrectReactionTimes * (double) (sumCorrectReactionTimes) / correctTrials)) / (correctTrials - 1.0));
	}
}
